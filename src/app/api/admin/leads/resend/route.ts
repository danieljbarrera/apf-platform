...
export async function POST(req: NextRequest) {
  ...
  const { data: quote, error } = await supabaseAdmin.from('quotes').select('*').eq('id', id).single();
  if (error || !quote) return NextResponse.json({ error: 'Quote not found' }, { status: 404 });
  if (!quote.email) return NextResponse.json({ error: 'No email on this lead' }, { status: 400 });

  const { first_name, last_name, email, event_date, guests, hours = 5, preferred_style, bar_package, appetizer_count = 0, include_dessert = false, include_coffee = false } = quote;
  ...
  const quoteId = `APF-RESEND-${Date.now().toString(36).toUpperCase()}`;
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  <tr><td style="background:${DARK};border-radius:10px 10px 0 0;padding:32px 36px;text-align:center;">
    ...
    <div style="font-size:26px;color:#fff;font-weight:400;margin-bottom:6px;">Your Estimate</div>
  ...
  <tr><td style="background:#fff;padding:28px 36px;border-left:1px solid #e7dfcf;border-right:1px solid #e7dfcf;">
    ...
    <p style="margin:0;font-size:14px;color:#3b382f;line-height:1.7;">Here is your custom estimate from All Purpose Flower.</p>
  ...
  <tr><td style="background:#fff;padding:0 36px 28px;border-left:1px solid #e7dfcf;border-right:1px solid #e7dfcf;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#ecf4ef;border:1px solid #c4dccd;border-radius:8px;padding:20px 24px;"><tr><td>
      <div style="font-size:16px;color:#38614a;margin-bottom:8px;">Ready to Move Forward?</div>
      <p style="font-size:13.5px;color:#3b382f;margin:0;line-height:1.7;">Reply to this email or reach us at info@allpurposeflowerco.com — we'd love to connect.</p>
    </td></tr></table>
  </td></tr>
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      subject: `Your All Purpose Flower Estimate`,
      html,
    });
  } catch (e) {
