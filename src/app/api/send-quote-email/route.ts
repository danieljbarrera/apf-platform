...
function customerHtml({ fname, lname, quoteId, eventType, eventDate, guests, hours, preferredStyle, packages, barQuote, barType, deposit }: Record<string, any>) {
  const styleOrder = [preferredStyle, ...['Buffet', 'Family Style', 'Plated'].filter((s: string) => s !== preferredStyle)];
  const pkgRows = styleOrder.map((style: string) => {
  ...
  <tr><td style="background:#fff;padding:0 36px 28px;border-left:1px solid #e7dfcf;border-right:1px solid #e7dfcf;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#ecf4ef;border:1px solid #c4dccd;border-radius:8px;padding:20px 24px;">
      <tr><td>
        ...
        <p style="font-size:13.5px;color:#3b382f;margin:0;line-height:1.7;">We'll reach out within 24 hours to talk through your vision, customize your menu, and answer every question.</p>
      </td></tr>
    </table>
  </td></tr>
...
function ownerHtml({ fname, lname, email, phone, quoteId, eventType, eventDate, venue, guests, hours, preferredStyle, appetizers, dessert, coffeeTea, barType, notes, packages, barQuote }: Record<string, any>) {
  const extras = [
  ...
  const styleRows = ['Buffet', 'Family Style', 'Plated'].map((s: string) => {
    const p = packages[s];
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <tr><td style="background:#fff;padding:24px 32px;border:1px solid #e7dfcf;border-top:none;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;width:130px;">Email</td><td style="padding:6px 0;font-size:13px;"><a href="mailto:${email}" style="color:${BRAND_COLOR};">${email}</a></td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Phone</td><td style="padding:6px 0;font-size:13px;">${phone || '—'}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Event type</td><td style="padding:6px 0;font-size:13px;">${eventType || '—'}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Event date</td><td style="padding:6px 0;font-size:13px;">${eventDate || '—'}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Venue</td><td style="padding:6px 0;font-size:13px;">${venue || '—'}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Guests</td><td style="padding:6px 0;font-size:13px;">${guests}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Duration</td><td style="padding:6px 0;font-size:13px;">${hours} hours</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Style</td><td style="padding:6px 0;font-size:13px;font-weight:600;">${preferredStyle}</td></tr>
      <tr><td style="padding:6px 0;font-size:13px;color:#79715f;">Extras</td><td style="padding:6px 0;font-size:13px;">${extras}</td></tr>
  ...
  <tr><td style="background:#fff;padding:0 32px 24px;border:1px solid #e7dfcf;border-top:none;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e7dfcf;border-radius:8px;overflow:hidden;border-collapse:separate;">
      ${styleRows}
      ${barQuote ? `<tr><td style="padding:7px 14px;">${barType}</td><td style="padding:7px 14px;text-align:right;font-weight:600;">${fmtD(barQuote.total)}</td></tr>` : ''}
    </table>
  </td></tr>
...
export async function POST(req: NextRequest) {
  ...
  const { fname, lname, email, phone, quoteId, eventType, eventDate, venue, guests, hours,
    preferredStyle, appetizers, dessert, coffeeTea, barType, notes, packages, barQuote } = body;
